...
const ZoneButton: React.FC<{ name: string; isActive: boolean; onClick: () => void }> = React.memo(
  ({ name, isActive, onClick }) => (
    <motion.button
      onClick={onClick}
      className={`font-orbitron px-6 py-3 rounded-full transition duration-300 ${
        isActive
...
const SpecItem: React.FC<{ icon: React.ElementType; label: string; value: string }> = ({
  icon: Icon,
  label,
  value,
}) => (
...
const ZoneDetails: React.FC<{ zone: (typeof zones)[0] }> = React.memo(({ zone }) => (
  <motion.div
    className="cyber-card overflow-hidden relative rounded-2xl border-2 border-neon-green"
  ...
  >
    ...
    <div className="absolute inset-0 p-8 flex flex-col justify-end">
      <motion.h3
        className="font-orbitron text-4xl font-bold mb-3 text-neon-green drop-shadow-lg"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      <motion.p
        className="text-white mb-6 text-xl font-medium drop-shadow-lg"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <motion.div
        className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 bg-dark-bg/60 p-6 rounded-xl backdrop-blur-sm"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <Link
          href="#booking"
          className="cyber-button inline-flex items-center space-x-2 text-lg px-8 py-3 rounded-full"
        >
...
const GameZones: React.FC = () => {
  const [activeZone, setActiveZone] = useState(0)
  const currentZone = useMemo(() => zones[activeZone], [activeZone])

  return (
    <section id="zones" className="py-24 bg-dark-bg">
      <div className="container mx-auto px-4">
        <motion.h2
          className="cyber-heading text-5xl mb-6 text-center"
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
